...
from HPTR.src.models.modules.point_net import PointNet
...
from HPTR.src.models.modules.decoder_ensemble import DecoderEnsemble, MLPHead
    ...
    def forward(
        ...
        for _ in range(inference_repeat_n):
            ...
            if self.pred_subsampling_rate != 1:
                ...
                pred = rearrange(
                    pred,
                    "n_decoder n_scene n_target n_pred n_step_future pred_dim -> (n_decoder n_scene n_target n_pred) "
                    "pred_dim n_step_future",
                )
                pred = F.interpolate(pred, mode="linear", scale_factor=self.pred_subsampling_rate)
                pred = rearrange(
                    pred,
                    "(n_decoder n_scene n_target n_pred) pred_dim n_step_future -> n_decoder n_scene n_target n_pred "
                    "n_step_future pred_dim",
    ...
    def __init__(
        ...
        if self.n_encoder_layers > 0:
            if self.n_latent_query > 0:
                self.latent_query = MultiModalAnchors(hidden_dim=hidden_dim, 
                                                      emb_dim=hidden_dim, 
...
class Decoder(nn.Module):
    ...
    def forward(self, valid: Tensor, target_type: Tensor, emb: Tensor, emb_invalid: Tensor) -> Tuple[Tensor, Tensor]:
        """
        Args:
            valid: [n_batch, n_target]
            emb_invalid: [n_batch * n_target, :]
            emb: [n_batch * n_target, :, hidden_dim]
            target_type: [n_batch, n_target, 3], bool one_hot [Vehicle=0, Pedestrian=1, Cyclist=2]

        Returns:
            conf: [n_batch, n_target, n_pred]
            pred: [n_batch, n_target, n_pred, n_step_future, pred_dim]
        """
        ...
        # [n_batch, n_target, n_pred, hidden_dim]
