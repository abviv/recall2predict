...
class WaymoPostProcessing(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        Note: During training, waymo_post_processing is skipped 
        """
        super().__init__()
        self.k_pred = k_pred
    ...
    def forward(self, pred_dict: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        if n_pred > self.k_pred:
            trajs, scores, selected_indices = self.mtr_nms(
        ...
        if "ref_idx" in pred_dict:
            ...
            scene_indices = torch.arange(n_scene, device=trajs.device).unsqueeze(1)
            ref_idx = pred_dict["ref_idx"]
            if ref_idx.device != trajs.device:
                ref_idx = ref_idx.to(trajs.device)
            waymo_trajs = torch.zeros(
                [n_scene, pred_dict["ref_idx_n"], self.k_pred, n_step, 2], device=trajs.device, dtype=trajs.dtype
            )
            waymo_trajs[scene_indices, ref_idx] = trajs
            ...
            waymo_scores[scene_indices, ref_idx] = scores.double()  # [n_scene, n_agent, k_pred]

            waymo_valid = torch.zeros([n_scene, pred_dict["ref_idx_n"]], device=trajs.device, dtype=torch.bool)
            pred_valid = pred_dict["pred_valid"].to(device=trajs.device, dtype=torch.bool)
            if pred_valid.numel() != ref_idx.numel():
                raise RuntimeError(
                    f"shape mismatch for pred_valid/ref_idx: pred_valid={tuple(pred_valid.shape)}, "
                    f"ref_idx={tuple(ref_idx.shape)}"
                )
            pred_valid = pred_valid.reshape_as(ref_idx)
            waymo_valid[scene_indices, ref_idx] = pred_valid
        ...
        if pred_dict["pred_yaw_bbox"] is not None:
            ...
            if selected_indices is not None:
                scene_idx = torch.arange(n_scene, device=yaw_bbox.device).view(-1, 1, 1)
                agent_idx = torch.arange(n_agent, device=yaw_bbox.device).view(1, -1, 1)
                yaw_bbox = yaw_bbox[scene_idx, agent_idx, selected_indices]
            if self.gt_in_local:
                yaw_bbox = torch_rad2global(yaw_bbox.flatten(2, 4), pred_dict["ref_yaw"])
    ...
    @staticmethod
    def mtr_nms(
        trajs: Tensor, scores: Tensor, k_pred: int, type_thresh: List[float], use_ade: bool, agent_type: Tensor
        ...
        Returns:
            ...
            mode_idx: [n_scene, n_agent, k_pred], indices of selected trajectories
        """
        # ! type dependent thresh
        thresh = 0
        for i in range(len(type_thresh)):
        ...
        scene_idx = torch.arange(scores.shape[0], device=scores.device).unsqueeze(1)  # [n_scene, 1]
        agent_idx = torch.arange(scores.shape[1], device=scores.device).unsqueeze(0)  # [1, n_agent]
        mode_idx = []
        ...
        return trajs_k, scores_k, mode_idx
