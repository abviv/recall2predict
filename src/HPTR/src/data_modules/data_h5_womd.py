...
from src.models.data_alignment.preprocess_probabilitymaps import generate_distance_transform, generate_probability_map
...
class DatasetTrainRaster(DatasetTrain):
    """
    ...
    def __getitem__(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        with h5py.File(self.raster_filepath, "r", libver="latest", swmr=True) as hf1:
            ...
            if scenario_key in hf1:
                ...
                out_dict["prob_map"] = np.ascontiguousarray(generate_probability_map(generate_distance_transform(raster_layer)))
                # out_dict["prob_map"] = np.ascontiguousarray(raster_layer)
        return out_dict
...
class DataH5womd(LightningDataModule):
    def __init__(
        ...
        filename_train_raster: str = "training_raster",
        filename_val_raster: str = "validation_raster",
        batch_size: int = 3,
        num_workers: int = 4,
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        if stage == "fit" or stage is None:
            if self.path_train_raster is not None:
                self.train_dataset = DatasetTrainRaster(self.path_train_h5, self.tensor_size_train, self.path_train_raster)
            else:
                self.train_dataset = DatasetTrain(self.path_train_h5, self.tensor_size_train)
    
            if self.path_val_raster is not None:
                self.val_dataset = DatasetValRaster(self.path_val_h5, self.tensor_size_val, self.path_val_raster)
            else:
    ...
    def custom_collate(batch):
        ...
        if isinstance(elem, dict):
            ...
            for key in elem:
                if key == 'prob_map' and isinstance(elem[key], (np.ndarray, torch.Tensor)):
                    # Store original dimensions of each probability map in the batch
                    orig_dims['gt/prob_map_orig_dims'] = torch.stack([
                        torch.tensor(d[key].shape[-2:], dtype=torch.long) for d in batch
                    ])

            # Normal collation
            result = {
                key: DataH5womd.custom_collate([d[key] for d in batch]) 
    ...
    def _get_dataloader(ds: Dataset, batch_size: int, num_workers: int) -> DataLoader[Any]:
        return DataLoader(
            ...
            collate_fn=DataH5womd.custom_collate
        )
