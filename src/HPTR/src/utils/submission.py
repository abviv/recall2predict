        ...
        for i_batch in range(waymo_trajs.shape[0]):
            ...
            for i_agent in range(agent_id.shape[0]):
                for n_K in self.submissions.keys():
                    confidence_slice = agent_score[i_agent, :n_K]
                    confidence_sum = confidence_slice.sum()
                    invalid_sum = (not np.isfinite(confidence_sum)) or (confidence_sum <= 1e-9)
                    invalid_values = ~np.isfinite(confidence_slice).all()
                    if invalid_sum or invalid_values:
                        print(
                            f"[SubAV2] Replacing invalid confidences for scenario {scenario_id[i_batch]} "
                            f"agent {int(agent_id[i_agent])} (sum={confidence_sum}, invalid_slice={invalid_values})."
                        )
                        confidence = np.full(n_K, 1.0 / n_K, dtype=confidence_slice.dtype)
                    else:
                        confidence = confidence_slice / confidence_sum
                    for _k in range(n_K):
                        self.submissions[n_K].append(
                            (
                                scenario_id[i_batch],
                                str(int(agent_id[i_agent])),
                                confidence[_k],
