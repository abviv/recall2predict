...
from src.HPTR.src.utils.transform_utils import torch_rad2rot, torch_pos2local, torch_dir2local, torch_rad2local
# from src.models.viz_components.visualization_hooks import MapVisualizer
...
class AgentCentricPreProcessing(nn.Module):
    def __init__(
        ...
        n_tl: int,
        mask_invalid: bool,
        visualize_world_data: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.visualize_world_data = visualize_world_data
        self._saved_debug_plot = False
        self.model_kwargs = {"gt_in_local": True, "agent_centric": True}

    @staticmethod
    def _plot_valid_trajectory(ax, pos: Tensor, valid: Tensor, color: str, linewidth: float, alpha: float, label: str) -> None:
        pos_np = pos.detach().to(dtype=torch.float32).cpu().numpy()
    ...
    def _save_single_agent_debug_plot(self, batch: Dict[str, Tensor]) -> None:
        if "gt/pos" not in batch or "gt/other_pos" not in batch:
            return

        scene_idx, target_idx = 0, 0
        save_path = Path("plots/debug/agent_centric_scene0_target0.png")
        save_path.parent.mkdir(parents=True, exist_ok=True)

        fig, ax = plt.subplots(figsize=(8, 8))

        self._plot_valid_trajectory(
            ax,
            batch["ac/target_pos"][scene_idx, target_idx],
            batch["ac/target_valid"][scene_idx, target_idx],
            color="green",
            linewidth=2.5,
            alpha=1.0,
            label="Focal history",
        ...
        self._plot_valid_trajectory(
            ax,
            batch["gt/pos"][scene_idx, target_idx],
            batch["gt/valid"][scene_idx, target_idx],
            color="red",
            linewidth=2.5,
            alpha=1.0,
            label="Focal future",
        ...
        n_other = batch["ac/other_pos"].shape[2]
        for other_idx in range(n_other):
            other_hist_label = "Other history" if other_idx == 0 else "_nolegend_"
            other_fut_label = "Other future" if other_idx == 0 else "_nolegend_"
            self._plot_valid_trajectory(
                ax,
                batch["ac/other_pos"][scene_idx, target_idx, other_idx],
                batch["ac/other_valid"][scene_idx, target_idx, other_idx],
                color="green",
                linewidth=1.2,
                alpha=0.35,
                label=other_hist_label,
            )
            self._plot_valid_trajectory(
                ax,
                batch["gt/other_pos"][scene_idx, target_idx, other_idx],
                batch["gt/other_valid"][scene_idx, target_idx, other_idx],
                color="red",
                linewidth=1.2,
                alpha=0.35,
                label=other_fut_label,
            )

        ax.scatter([0.0], [0.0], c="black", s=50, marker="x", label="Focal current", zorder=10)
        ax.set_title("Agent-Centric Trajectories: Scene 0, Target 0")
        ax.set_xlabel("x (target-local)")
        ax.set_ylabel("y (target-local)")
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        Args: scene-centric Dict
            ...
            "map/boundary": [n_scene, 4], float32 [xmin, xmax, ymin, ymax] in SDC-centered coords
            "gt/scenario_center": [n_scene, 2], float32 SDC center in True World coords
            "gt/scenario_yaw": [n_scene], float32 SDC yaw in True World coords
        ...
        Returns: agent-centric Dict, masked according to valid
            ...
            # (gt) ground-truth target future for training, not available for testing
                ...
                "gt/other_valid": [n_scene, n_target, n_other, n_step_future], bool
                "gt/other_pos": [n_scene, n_target, n_other, n_step_future, 2]
                "gt/other_vel": [n_scene, n_target, n_other, n_step_future, 2]
                "gt/sdf_map": [n_scene, x, y] signed distance field of the raster in meters
                "gt/sim2_R": [n_scene, 2, 2]
                "gt/sim2_s": [n_scene, 1]
                "gt/sim2_t": [n_scene, 2]
            ...
            # target agents attributes
                "ac/target_type": [n_scene,t n_target, 3]
        ...
        device = batch[prefix + "agent/valid"].device # Get device

        # --- Keep track of original SDC-centered map data for potential plotting ---
        original_map_pos_sdc = batch["map/pos"].clone() if "map/pos" in batch else None
        original_map_valid_sdc = batch["map/valid"].clone() if "map/valid" in batch else None
        # -------------------------------------------------------------------------
        ...
        batch["ref/yaw"] = ref_yaw  # [n_scene, n_target]
        ...
        if "agent/valid" in batch.keys():
            ...
            # Add random raster ground truth data
            batch["gt/sdf_map"] = batch.get("sdf_map")
            batch["gt/sim2_R"] = batch.get("sim2_R")
            batch["gt/sim2_t"] = batch.get("sim2_t")
            batch["gt/sim2_s"] = batch.get("sim2_s")
            batch["gt/scenario_center"] = batch.get("scenario_center")
            batch["gt/scenario_yaw"] = batch.get("scenario_yaw")
        ...
        other_is_within_range = other_dist.unsqueeze(-1) < 2e3
        ...
        batch["ac/other_valid"] = batch["ac/other_valid"] & other_is_within_range
        ...
        # ! prepare ground-truth future for selected other agents
        # [n_scene, n_step, n_agent, ...] -> [n_scene, n_target, n_other, n_step_future, ...]
        if "agent/valid" in batch.keys():
            batch["gt/other_valid"] = (
                batch["agent/valid"][:, self.n_step_hist :]
                .transpose(1, 2)
                .unsqueeze(1)
                .repeat(1, self.n_target, 1, 1)[other_scene_indices, other_target_indices, other_indices]
            )
            batch["gt/other_valid"] = batch["gt/other_valid"] & other_is_within_range
            for k in ("pos", "vel"):
                batch[f"gt/other_{k}"] = (
            ...
            batch["gt/other_pos"] = torch_pos2local(batch["gt/other_pos"], ref_pos.unsqueeze(2), ref_rot.unsqueeze(2))
            batch["gt/other_vel"] = torch_dir2local(batch["gt/other_vel"], ref_rot.unsqueeze(2))
        ...
        # self._save_single_agent_debug_plot(batch)
        # self._saved_debug_plot = True
