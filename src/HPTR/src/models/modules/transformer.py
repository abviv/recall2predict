...
def _check_nan(tensor: Optional[Tensor], name: str, location: str) -> None:
    if tensor is not None and tensor.isnan().any():
        n_nan = tensor.isnan().sum().item()
        total = tensor.numel()
        print(f"[NaN] {location} | {name}: {n_nan}/{total} NaN values — shape {tuple(tensor.shape)}")
...
class TransformerCrossAttention(nn.Module):
    ...
    def forward(
        ...
        """
        # --- NaN checks: inputs ---
        _check_nan(src, "src", "TransformerCrossAttention input")
        _check_nan(tgt, "tgt", "TransformerCrossAttention input")
        _check_nan(rpe, "rpe", "TransformerCrossAttention input")
        _check_nan(decoder_tgt, "decoder_tgt", "TransformerCrossAttention input")
        _check_nan(decoder_rpe, "decoder_rpe", "TransformerCrossAttention input")
        ...
        if self.decoder_self_attn:
            ...
            # --- NaN check: after decoder self-attn ---
            _check_nan(src, "src", "TransformerCrossAttention after decoder self-attn")
        ...
        # --- NaN check: after cross/self-attn ---
        _check_nan(src2, "src2", "TransformerCrossAttention after cross/self-attn")
        ...
        # --- NaN check: after feedforward ---
        _check_nan(src, "src", "TransformerCrossAttention after feedforward")
